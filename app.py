...
class User(UserMixin, db.Model):
    ...
    telegram_first_name = db.Column(db.String(120), nullable=True)
    telegram_last_name = db.Column(db.String(120), nullable=True)
...
class Notification(db.Model):
    """Personal user notification feed."""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    task_id = db.Column(db.Integer, nullable=True)
...
class TeamInvite(db.Model):
    """Invite link for scalable team onboarding."""
    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(64), unique=True, nullable=False)
    team_id = db.Column(db.Integer, db.ForeignKey("team.id"), nullable=False)
    role = db.Column(db.String(20), default="member", nullable=False)
    max_uses = db.Column(db.Integer, default=25)
    ...
    team = db.relationship("Team", backref=db.backref("invites", cascade="all, delete-orphan"), lazy=True)
    creator = db.relationship("User", foreign_keys=[created_by], lazy=True)


class TelegramLoginCode(db.Model):
    """One-time code generated by Telegram bot for local-friendly Telegram auth."""
    id = db.Column(db.Integer, primary_key=True)
    code_hash = db.Column(db.String(128), unique=True, nullable=False)
...
def seed_database() -> None:
    ...
    # Useful demo automation rules for the jury presentation.
    if admin and not Rule.query.first():
        db.session.add(Rule(
            name="Critical tasks go to In Progress",
            trigger="task_created",
            condition_field="priority",
            condition_operator="equals",
            condition_value="critical",
            action_type="move_column",
            action_value="in_progress",
            created_by=admin.id,
        ))
        db.session.add(Rule(
            name="Bug tag becomes high priority",
            trigger="incoming_task",
    ...
    try:
        ...
        if "task" in tables:
            ...
            if "version" not in columns:
                connection.execute("ALTER TABLE task ADD COLUMN version INTEGER DEFAULT 1 NOT NULL")
                print("[DB] Added task.version")
        connection.commit()
    finally:
        connection.close()
...
def migrate_core_schema() -> None:
    """Small cross-DB migration for hackathon releases.

    Keeps old Docker volumes working when we add demo features without Alembic.
    """
    try:
        ...
        for column, ddl in column_defs.items():
            if column not in user_columns:
                db.session.execute(text(f"ALTER TABLE {user_table} ADD COLUMN {column} {ddl}"))
                print(f"[DB] Added user.{column}")
        db.session.commit()
    except Exception as exc:  # noqa: BLE001
        db.session.rollback()
        print(f"[DB] Lightweight schema migration skipped: {exc}")


# ===================== HELPERS =====================

def get_request_user() -> User | None:
    """Return current Flask-Login user only inside an HTTP request.

    Background processes (worker.py, automation_worker.py, outbox_publisher.py)
    import app.py and execute helper functions without a browser session.
    In that context current_user must not be accessed directly.
    """
...
def can_edit_task(task: Task) -> bool:
    ...
    if task.team_id and is_team_admin(task.team_id, user):
        return True
...
def can_view_task(task: Task) -> bool:
    ...
    if task.team_id:
        return is_team_member(task.team_id, user.id)

    # Tasks without a team are personal by default.
    return False
...
def validate_team_assignment(team: Team | None, assignee: User | None, changing_assignment: bool = True) -> tuple[bool, str]:
    ...
    In HTTP routes we check permissions of the current logged-in user.
    In background workers there is no current_user, so we only validate data
    consistency, for example that assignee belongs to the selected team.
    """
    user = get_request_user()

    if not user:
        if team and assignee and not is_team_member(team.id, assignee.id):
            return False, "Исполнитель должен быть участником выбранной команды"
        return True, ""

    if team:
        if not is_global_admin(user) and not is_team_member(team.id, user.id):
            return False, "Вы не состоите в этой команде"
        if assignee and not is_team_member(team.id, assignee.id):
            return False, "Исполнитель должен быть участником выбранной команды"
        if assignee and assignee.id != user.id and not is_team_admin(team.id, user):
            return False, "Назначать задачи другим может только админ команды"
        return True, ""

    if assignee and assignee.id != user.id and not is_global_admin(user):
...
def get_visible_tasks() -> list[Task]:
    ...
    if not is_global_admin(user):
        team_ids = get_user_team_ids(user)
        # Personal tasks are visible only to creator/assignee.
        # Team tasks are visible to members of that team.
        filters = [Task.creator_id == user.id, Task.assignee_id == user.id]
        if team_ids:
...
def serialize_team(team: Team) -> dict[str, Any]:
    return {
        ...
        "members": [
            serialize_team_member(m)
            for m in TeamMember.query.filter_by(team_id=team.id).order_by(TeamMember.role.asc()).all()
        ],
    }
...
def serialize_task(task: Task) -> dict[str, Any]:
    ...
    return {
        ...
        "can_edit": can_edit_task(task) if user else False,
...
def serialize_invite(invite: TeamInvite) -> dict[str, Any]:
    expired = bool(invite.expires_at and invite.expires_at < datetime.utcnow())
    return {
        "id": invite.id,
        "team_id": invite.team_id,
        "team_name": invite.team.name if invite.team else "",
        "token": invite.token,
        "role": invite.role,
        "uses": invite.uses,
        "max_uses": invite.max_uses,
        "active": invite.active and not expired and invite.uses < invite.max_uses,
        "expires_at": invite.expires_at.strftime("%Y-%m-%d %H:%M:%S") if invite.expires_at else "",
        "url": url_for("join_invite", token=invite.token, _external=True),
...
def get_dashboard_stats() -> dict[str, Any]:
    user = get_request_user()
    tasks = get_visible_tasks() if user else []
    today = date.today()

    return {
        "tasks_total": len(tasks),
        "tasks_done": sum(1 for task in tasks if task.column_ref and task.column_ref.key == "done"),
        "tasks_overdue": sum(
            1 for task in tasks
            if task.deadline and task.deadline < today and task.column_ref and task.column_ref.key != "done"
        ),
        "tasks_critical": sum(1 for task in tasks if task.priority == "critical"),
        "teams_total": len(get_visible_teams()) if user else 0,
        "unread_notifications": Notification.query.filter_by(user_id=user.id, is_read=False).count() if user else 0,
        "incoming_pending": IncomingTask.query.filter(IncomingTask.status.in_(["pending", "queued"])).count(),
        "incoming_errors": IncomingTask.query.filter_by(status="error").count(),
    }


def get_visible_invites() -> list[TeamInvite]:
    user = get_request_user()
    if not user:
        return []

    query = TeamInvite.query.order_by(TeamInvite.created_at.desc()).limit(50)
    if is_global_admin(user):
        return query.all()

    team_ids = [team.id for team in get_visible_teams() if can_manage_team(team.id)]
    if not team_ids:
        return []

    return TeamInvite.query.filter(TeamInvite.team_id.in_(team_ids)).order_by(TeamInvite.created_at.desc()).limit(50).all()
...
def get_state() -> dict[str, Any]:
    ...
    return {
        ...
        "tasks": [serialize_task(t) for t in get_visible_tasks()] if user else [],
        "teams": [serialize_team(t) for t in get_visible_teams()] if user else [],
        "users": [serialize_user(u) for u in get_visible_users()] if user else [],
        "rules": [serialize_rule(r) for r in Rule.query.order_by(Rule.created_at.desc()).all()] if is_global_admin(user) else [],
        "events": [serialize_event(e) for e in Event.query.order_by(Event.created_at.desc()).limit(50).all()],
        "incoming": [serialize_incoming(i) for i in IncomingTask.query.order_by(IncomingTask.created_at.desc()).limit(30).all()],
        "notifications": [
            serialize_notification(n)
            for n in Notification.query.filter_by(user_id=user.id).order_by(Notification.created_at.desc()).limit(30).all()
        ] if user else [],
        "invites": [serialize_invite(inv) for inv in get_visible_invites()] if user else [],
...
def get_status_counts(model, status_column: str = "status") -> dict[str, int]:
    """Return grouped counts by status for admin dashboards."""
    column = getattr(model, status_column)
    rows = db.session.query(column, db.func.count(model.id)).group_by(column).all()
...
def get_rabbitmq_queue_stats() -> dict[str, Any]:
    ...
    queue_names = [
        ...
        "incoming_tasks_dead",
        "automation_events",
        "automation_events_dead",
    ]
    stats: dict[str, Any] = {"ok": False, "queues": {}, "error": ""}
    try:
        ...
        for queue_name in queue_names:
            try:
                ...
                stats["queues"][queue_name] = {"messages": None, "consumers": None, "error": str(queue_exc)}
        connection.close()
        stats["ok"] = True
    except Exception as exc:  # noqa: BLE001
        stats["error"] = str(exc)
...
def get_system_health_payload() -> dict[str, Any]:
    """Admin-facing health snapshot for demo and troubleshooting."""
    health: dict[str, Any] = {
    ...
    try:
        ...
        health["database"] = {"ok": True, "message": "PostgreSQL доступен"}
    except Exception as exc:  # noqa: BLE001
        health["database"] = {"ok": False, "message": str(exc)}
        health["recommendations"].append("Проверить контейнер postgres и DATABASE_URL")

    rabbit_stats = get_rabbitmq_queue_stats()
    health["rabbitmq"] = rabbit_stats
    if not rabbit_stats.get("ok"):
        health["recommendations"].append("Проверить контейнер rabbitmq и RABBITMQ_URL")

    redis_url_local = os.environ.get("REDIS_URL", "")
    if redis_url_local:
        try:
            client = redis.Redis.from_url(redis_url_local, socket_connect_timeout=2, socket_timeout=2)
            pong = client.ping()
            health["redis"] = {"ok": bool(pong), "message": "Redis доступен"}
        except Exception as exc:  # noqa: BLE001
            health["redis"] = {"ok": False, "message": str(exc)}
            health["recommendations"].append("Проверить контейнер redis и REDIS_URL")
    else:
        health["redis"] = {"ok": None, "message": "REDIS_URL не задан; Socket.IO работает только в одном процессе"}

    incoming_counts = get_status_counts(IncomingTask)
    outbox_counts = get_status_counts(OutboxMessage)
    health["counters"] = {
        "tasks": Task.query.count(),
        "teams": Team.query.count(),
        "users": User.query.count(),
        "rules_enabled": Rule.query.filter_by(enabled=True).count(),
    ...
    if incoming_counts.get("error", 0) > 0:
        health["recommendations"].append("Есть ошибки во входящем потоке: откройте Очередь и нажмите Retry")
    if outbox_counts.get("failed", 0) > 0:
        health["recommendations"].append("Есть failed outbox-сообщения: проверьте RabbitMQ и логи outbox")

    services_ok = bool(health["database"].get("ok")) and bool(health["rabbitmq"].get("ok")) and health["redis"].get("ok") is not False
    health["services"] = {
        "overall": "ok" if services_ok else "warning",
        "web": "running",
        "workers": "см. Docker Compose / логи worker, automation, outbox",
    }
    return health


def ensure_prime_demo_data() -> dict[str, Any]:
    """Create a polished demo scenario without deleting user data."""
    admin = User.query.filter_by(username="admin").first() or get_request_user()
    demo_user = User.query.filter_by(username="user").first()
    todo = Column.query.filter_by(key="todo").first() or get_default_column()
    in_progress = Column.query.filter_by(key="in_progress").first() or todo
    done = Column.query.filter_by(key="done").first() or todo

    team = Team.query.filter_by(key="prime_demo_squad").first()
    if not team:
        team = Team(
    ...
    if admin:
        ensure_team_member(team, admin, "admin")
    if demo_user:
        ensure_team_member(team, demo_user, "member")
    ...
    task_specs = [
        ...
        {
            "title": "Critical: проверить оплату через входящий поток",
            "description": "Задача демонстрирует обогащение urgent/bug и реакцию правил.",
            "priority": "critical",
        ...
        {
            "title": "Оформить README и инструкции запуска",
            "description": "README должен объяснять архитектуру, очередь и деплой.",
            "priority": "medium",
            "tags": "docs, readme",
            "column": done,
            "assignee": admin,
    ...
    for spec in task_specs:
        task = Task.query.filter_by(title=spec["title"]).first()
        if task:
            continue
        task = Task(
            title=spec["title"],
            ...
            column_id=spec["column"].id,
            creator_id=admin.id if admin else None,
            team_id=team.id,
            assignee_id=spec["assignee"].id if spec["assignee"] else None,
        )
        db.session.add(task)
        db.session.flush()
        log_event("demo_task_created", task.id, {"title": task.title})
        notify_assignment(task)
        publish_task_event("task_created", task, {"created_from": "prime_demo"})
        created_tasks += 1

    if not Rule.query.filter_by(name="Prime: urgent tasks become critical").first():
        db.session.add(Rule(
            name="Prime: urgent tasks become critical",
            trigger="incoming_task",
            condition_field="tags",
            condition_operator="contains",
    ...
    incoming_payload = {
        ...
        "title": "Prime demo: задача из внешнего API",
        "description": "Сгенерировано кнопкой Demo Mode. Должно пройти через outbox → RabbitMQ → worker.",
        "tags": "api, urgent, demo",
        "team": team.name,
        "assignee": demo_user.username if demo_user else "",
    }
    item = IncomingTask(
        source="prime_demo",
        external_id=external_id,
        payload=json.dumps(incoming_payload, ensure_ascii=False),
        status="queued",
    )
    db.session.add(item)
    db.session.flush()
    outbox = OutboxMessage(
        ...
        payload=json.dumps({"message_id": f"incoming-task:{item.id}:demo:{uuid.uuid4().hex}", "incoming_task_id": item.id}, ensure_ascii=False),
        status="pending",
    )
    db.session.add(outbox)
    log_event("prime_demo_created", None, {"team": team.name, "incoming_id": item.id, "created_tasks": created_tasks})
    db.session.commit()
    emit_state()

    return {"team_id": team.id, "created_tasks": created_tasks, "incoming_task_id": item.id}

def emit_state() -> None:
    """Broadcast only a lightweight signal, never user-specific state.

    Workers have no browser session/current_user. Each browser receives this
    event and then calls /api/state itself. The backend returns only data that
    this exact logged-in user is allowed to see.
    """
...
def enqueue_outbox_message(routing_key: str, payload: dict[str, Any], exchange: str = "kanban") -> OutboxMessage:
    """Create an outbox row inside the current SQL transaction.

    This is the transactional-outbox pattern: web/worker code writes DB state and
    the message-to-publish in one commit. outbox_publisher.py later sends it to RabbitMQ.
    """
...
def publish_task_event(trigger: str, task: Task, extra: dict[str, Any] | None = None) -> None:
    """Queue a task-domain event for automation_worker.py.

    Routes do not execute automation synchronously anymore. They save state, write
    an event log, and enqueue a RabbitMQ event. The automation worker consumes it,
    checks rules and applies actions.
    """
    routing_map = {
        "task_created": "task.created",
        "task_updated": "task.updated",
        "task_moved": "task.moved",
        "incoming_task": "incoming.task.processed",
    }
    routing_key = routing_map.get(trigger)
    if not routing_key:
        return

    payload = {
        "message_id": f"automation:{trigger}:{task.id}:{uuid.uuid4().hex}",
        "trigger": trigger,
        "task_id": task.id,
        "extra": extra or {},
    }
    enqueue_outbox_message(routing_key, payload)
...
def send_telegram_message_to_user(user_id: int | None, message: str, task_id: int | None = None) -> bool:
    """Send a personal Telegram message to a linked user.

    This function is intentionally safe for background workers:
    it never uses current_user/request context and never raises if Telegram is unavailable.
    """
    ...
    public_base_url = os.environ.get("PUBLIC_BASE_URL", "").strip().rstrip("/")
    if public_base_url:
        text_message += f"\nОткрыть NineCell: {public_base_url}"
    ...
    try:
        ...
        print(f"[Telegram notify] Failed to send message to user_id={user_id}: {exc}")
        return False
...
def notify_user(
    ...
    flash_to_all: bool = False,
    send_telegram: bool = False,
) -> None:
    """Persist a personal notification and optionally send it to Telegram.

    `send_telegram=True` is used for assignment notifications, so the assignee
    receives a message in Telegram if their account is linked to the bot.
    """
    if not user_id:
        return

    db.session.add(Notification(user_id=user_id, task_id=task_id, message=message, type=notif_type))

    if send_telegram:
        send_telegram_message_to_user(user_id, message, task_id)

    if flash_to_all:
        send_notification(message, notif_type)
    else:
        # Lightweight signal; every browser reloads only its permitted state.
        socketio.emit("state_changed", {"time": datetime.now().strftime("%H:%M:%S")})


def notify_assignment(task: Task, previous_assignee_id: int | None = None) -> None:
    if task.assignee_id and task.assignee_id != previous_assignee_id:
        notify_user(
            task.assignee_id,
            f'Вам назначена задача #{task.id}: "{task.title}"',
            "success",
            task.id,
            send_telegram=True,
        )
...
def process_incoming_task(item: IncomingTask) -> None:
    try:
        ...
        task = Task(
            ...
            column_id=column.id,
            creator_id=request_user.id if request_user else None,
...
def generate_login_code() -> str:
    # 6 digits are easy to type during demo. Hash is stored in DB, raw code is shown once by the bot.
    return f"{secrets.randbelow(1_000_000):06d}"


def build_unique_username(base: str) -> str:
    base = slugify(base).replace("_", "") or "telegram_user"
    base = base[:42]
    candidate = base
    suffix = 1
...
def get_or_create_telegram_user(profile: dict[str, Any]) -> User:
    telegram_id = str(profile.get("telegram_id") or profile.get("id") or "").strip()
    if not telegram_id:
        raise ValueError("telegram_id is required")

    user = User.query.filter_by(telegram_id=telegram_id).first()
    if user:
        user.telegram_username = profile.get("telegram_username") or profile.get("username") or user.telegram_username
        user.telegram_first_name = profile.get("first_name") or user.telegram_first_name
        user.telegram_last_name = profile.get("last_name") or user.telegram_last_name
        user.telegram_photo_url = profile.get("photo_url") or user.telegram_photo_url
        return user

    username_source = profile.get("telegram_username") or profile.get("username") or profile.get("first_name") or f"tg_{telegram_id}"
    username = build_unique_username(str(username_source))
    user = User(
        username=username,
        role="user",
        ...
        telegram_photo_url=profile.get("photo_url"),
    )
    user.set_password(secrets.token_urlsafe(32))
    db.session.add(user)
    db.session.flush()
    log_event("telegram_user_registered", None, {"username": user.username, "telegram_id": telegram_id})
    return user


def verify_telegram_widget_data(args: dict[str, Any]) -> tuple[bool, str]:
    """Legacy Telegram Login Widget hash check.

    Production note: Telegram also has an OIDC login flow. For the hackathon we keep
    the legacy widget plus local-friendly one-time bot code.
    """
    if not TELEGRAM_BOT_TOKEN:
        return False, "TELEGRAM_BOT_TOKEN не задан"
    received_hash = str(args.get("hash") or "")
    if not received_hash:
        return False, "Нет hash от Telegram"
    data = {k: str(v) for k, v in args.items() if k != "hash" and v is not None and str(v) != ""}
...
def login():
    ...
    return render_template("login.html", error=error, telegram_bot_username=TELEGRAM_BOT_USERNAME)
...
def register():
    ...
    return render_template("register.html", error=error, telegram_bot_username=TELEGRAM_BOT_USERNAME)



@app.route("/auth/telegram/code", methods=["POST"])
def telegram_code_login():
    if current_user.is_authenticated:
        return redirect(url_for("index"))

    code = request.form.get("telegram_code", "").strip().replace(" ", "")
    next_url = request.args.get("next")
    if not re.fullmatch(r"\d{6}", code):
        return render_template("login.html", error="Введите 6-значный код из Telegram", telegram_error="Введите 6-значный код из Telegram", telegram_bot_username=TELEGRAM_BOT_USERNAME), 400

    code_hash = hash_login_code(code)
    item = TelegramLoginCode.query.filter_by(code_hash=code_hash).first()
    if not item or item.used_at or item.expires_at < datetime.utcnow():
        return render_template("login.html", error="Telegram-код недействителен или истёк", telegram_error="Telegram-код недействителен или истёк", telegram_bot_username=TELEGRAM_BOT_USERNAME), 400
    ...
    log_event("telegram_login", None, {"username": user.username, "telegram_id": item.telegram_id})
    db.session.commit()
    login_user(user)
    return redirect(next_url or url_for("index"))


@app.route("/auth/telegram/widget")
def telegram_widget_login():
    if current_user.is_authenticated:
        return redirect(url_for("index"))

    ok, error = verify_telegram_widget_data(request.args.to_dict())
    if not ok:
        return render_template("login.html", error=error, telegram_error=error, telegram_bot_username=TELEGRAM_BOT_USERNAME), 400
    ...
    log_event("telegram_widget_login", None, {"username": user.username, "telegram_id": user.telegram_id})
    db.session.commit()
    login_user(user)
    return redirect(url_for("index"))


@app.route("/api/telegram_auth/request_code", methods=["POST"])
def api_telegram_auth_request_code():
    api_key = request.headers.get("X-API-Key", "")
    if api_key != os.environ.get("INCOMING_API_KEY", "dev-incoming-token"):
    ...
    login_code = TelegramLoginCode(
        code_hash=hash_login_code(code),
        telegram_id=telegram_id,
        telegram_username=(data.get("telegram_username") or data.get("username") or "").strip() or None,
        first_name=(data.get("first_name") or "").strip() or None,
        last_name=(data.get("last_name") or "").strip() or None,
        photo_url=(data.get("photo_url") or "").strip() or None,
        expires_at=datetime.utcnow() + timedelta(seconds=TELEGRAM_LOGIN_TTL_SECONDS),
    )
    db.session.add(login_code)
    db.session.commit()
    return jsonify({"status": "ok", "code": code, "expires_in": TELEGRAM_LOGIN_TTL_SECONDS})
...
def add_task():
    ...
    notify_assignment(task)
    publish_task_event("task_created", task, {"created_from": "web"})
    db.session.commit()
...
def update_task(task_id: int):
    ...
    if "version" in data and int(data.get("version") or 0) != (task.version or 1):
        return jsonify({
            "status": "conflict",
            "message": "Задача уже была изменена другим пользователем. Обновите страницу и повторите действие.",
    ...
    task.version = (task.version or 1) + 1
    notify_assignment(task, previous_assignee_id)
    log_event("task_updated", task.id, {"before": old_data, "after": serialize_task(task)})
    publish_task_event("task_updated", task)
    db.session.commit()
...
def move_task(task_id: int):
    ...
    if "version" in data and int(data.get("version") or 0) != (task.version or 1):
        return jsonify({
            "status": "conflict",
            "message": "Задача уже была изменена другим пользователем. Доска обновится автоматически.",
            "current_version": task.version or 1,
        }), 409
    new_column = find_column(data.get("column_id")) or find_column(data.get("column"))
    if not new_column:
    ...
    publish_task_event("task_moved", task, {"from": old_column, "to": new_column.name})
    db.session.commit()
...
def update_team(team_id: int):
    ...
    if not can_manage_team(team.id):
        return jsonify({
            "status": "error",
            "message": "Нужны права админа команды"
        }), 403
    ...
    send_notification(f'✏️ Команда обновлена: "{team.name}"', "success")
    emit_state()
...
def delete_team(team_id: int):
    ...
    # Важно: задачи НЕ удаляем.
    # Просто делаем их личными/без команды и снимаем исполнителей.
    ...
    send_notification(f'🗑️ Команда удалена: "{team_name}"', "warning")
    emit_state()
...
# ===================== API: TEAM INVITES & NOTIFICATIONS =====================

@app.route("/create_invite/<int:team_id>", methods=["POST"])
@login_required
def create_invite(team_id: int):
    team = Team.query.get_or_404(team_id)
    if not can_manage_team(team.id):
        return jsonify({"status": "error", "message": "Нужны права админа команды"}), 403
    ...
    if role not in {"admin", "member"}:
        return jsonify({"status": "error", "message": "Роль должна быть admin или member"}), 400
    max_uses = int(data.get("max_uses") or 25)
    ...
    db.session.flush()
    log_event("team_invite_created", None, {"team": team.name, "role": role, "max_uses": max_uses})
    db.session.commit()
    emit_state()
    return jsonify({"status": "ok", "invite": serialize_invite(invite)})


@app.route("/disable_invite/<int:invite_id>", methods=["POST"])
@login_required
def disable_invite(invite_id: int):
    invite = TeamInvite.query.get_or_404(invite_id)
    if not can_manage_team(invite.team_id):
        return jsonify({"status": "error", "message": "Нужны права админа команды"}), 403
    invite.active = False
    log_event("team_invite_disabled", None, {"team_id": invite.team_id, "invite_id": invite.id})
    db.session.commit()
    emit_state()
    return jsonify({"status": "ok"})
...
def join_invite(token: str):
    ...
    expired = bool(invite.expires_at and invite.expires_at < datetime.utcnow())
    if not invite.active or expired or invite.uses >= invite.max_uses:
        return render_template("join_result.html", ok=False, message="Ссылка приглашения недействительна или истекла")

    ensure_team_member(invite.team, current_user, invite.role)
    invite.uses += 1
    log_event("team_invite_joined", None, {"team": invite.team.name, "user": current_user.username, "role": invite.role})
    notify_user(invite.created_by, f'{current_user.username} вступил(а) в команду {invite.team.name}', "info")
    db.session.commit()
    emit_state()
    return render_template("join_result.html", ok=True, message=f'Вы вступили в команду "{invite.team.name}" как {invite.role}')
...
def mark_notifications_read():
    ...
    if ids:
        query = query.filter(Notification.id.in_([int(i) for i in ids]))
    updated = 0
...
def clear_notifications():
    ...
    By default it clears all personal notifications.
    If request JSON contains {"only_read": true}, it clears only already-read notifications.
    """
    data = request.get_json() or {}
    query = Notification.query.filter_by(user_id=current_user.id)
    if data.get("only_read"):
...
def enqueue_incoming_payload(data: dict[str, Any]):
    """Persist raw incoming task + outbox message in one DB commit.

    Used by the public API endpoint, Telegram bot and the authenticated UI demo.
    The actual Task is created asynchronously by worker.py after RabbitMQ delivery.
    """
    title = str(data.get("title", "")).strip()
    if not title:
    ...
    # Light duplicate guard before queueing. Worker repeats deduplication safely.
    if external_id:
...
def incoming_tasks():
    """External API endpoint protected by X-API-Key."""
    if not api_key_is_valid():
        return jsonify({"status": "error", "message": "Invalid or missing X-API-Key"}), 401
    return enqueue_incoming_payload(request.get_json() or {})


@app.route("/api/incoming_tasks/ui", methods=["POST"])
@login_required
def incoming_tasks_ui():
    """Authenticated UI endpoint for demoing the queue without exposing X-API-Key."""
    data = request.get_json() or {}
    data.setdefault("source", "ui")
    return enqueue_incoming_payload(data)



# ===================== API: PRIME ADMIN TOOLS =====================
...
@login_required
def api_task_activity(task_id: int):
    task = Task.query.get_or_404(task_id)
    if not can_view_task(task):
        return jsonify({"status": "error", "message": "Нет доступа к этой задаче"}), 403
    events = Event.query.filter_by(task_id=task.id).order_by(Event.created_at.desc()).limit(80).all()
    return jsonify({"status": "ok", "task": serialize_task(task), "events": [serialize_event(e) for e in events]})


@app.route("/api/incoming_tasks/<int:incoming_id>/retry", methods=["POST"])
@login_required
@admin_required
def retry_incoming_task(incoming_id: int):
    item = IncomingTask.query.get_or_404(incoming_id)
    if item.status not in {"error", "duplicate", "processed"}:
        return jsonify({"status": "error", "message": "Повторить можно только error/duplicate/processed записи"}), 400
    ...
    message_id = f"incoming-task:{item.id}:retry:{uuid.uuid4().hex}"
    outbox = OutboxMessage(
        exchange="kanban",
        routing_key="incoming.task.created",
        payload=json.dumps({"message_id": message_id, "incoming_task_id": item.id}, ensure_ascii=False),
        status="pending",
    )
    db.session.add(outbox)
    log_event("incoming_task_retry_requested", None, {"incoming_id": item.id, "message_id": message_id})
    db.session.commit()
    emit_state()
    return jsonify({"status": "ok", "incoming_task_id": item.id, "outbox_message_id": outbox.id})


@app.route("/api/demo/prime", methods=["POST"])
@login_required
@admin_required
def api_prime_demo():
    result = ensure_prime_demo_data()
    send_notification("🚀 Prime demo-сценарий создан: задачи, команда, правило и входящая очередь", "success")
